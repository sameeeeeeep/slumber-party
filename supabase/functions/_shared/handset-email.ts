// ============================================================================
//  The handset email — the Y2K candybar every message from khushi arrives on.
//
//  This markup used to live inside submit/index.ts, which was fine while there
//  was one email. There are two now (the application confirmation, and the
//  waitlist note for people who landed on /late after the deadline), and two
//  copies of a 250-line table layout drift the moment anyone touches the logo,
//  the palette or the keypad. So the HANDSET lives here and the WORDS live at
//  the call site: everything that differs between the two emails is a field on
//  HandsetCopy, and everything that doesn't is below and is not duplicated.
//
...
//
//  Nothing here reads Deno.env at import time, so the template can be rendered
//  outside the edge runtime (see tools/render-emails.mjs) to eyeball a change
//  before it ships to anyone's inbox.
// ============================================================================
...
export const LOGO_CID = 'slumberlogo';

export const escapeHtml = (s: string) =>
  s.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');

/** Everything that differs between one handset email and the next. */
export interface HandsetCopy {
  /** Inbox subject. No emoji: this domain has little sending history and some
   *  filters still score emoji subjects, which is a free thing to skip. */
  subject: string;
  /** The grey line beside the subject in the inbox list. Plain text/entities. */
  preview: string;
  /** Rendered as "hey <name> ♥" on the LCD. Escaped here, so pass it raw. */
  greetingName: string;
  /** LCD body lines, in order. HTML entities allowed; these are NOT escaped,
   *  because the copy leans on &#8212; and friends. Keep them SMS-short — the
   *  screen has to stay small or the phone stops looking like a phone. */
  lines: string[];
  /** The one dark key under the screen. */
  cta: { label: string; href: string };
  /** Footer, first line: "<reason><br>secretslumberparty.com ♥". */
  footerReason: string;
  /** The plain-text twin. Same words, no markup — an HTML-only email is a spam
   *  signal by itself, so this is not optional. */
  text: string;
}

export function handsetEmail(copy: HandsetCopy): { subject: string; html: string; text: string } {
  /* The "real time" of this design. No JS runs in a mail client, but the HTML is
     ...
     the minute THIS person hit send. Everything is IST — the party is in Mumbai
     and so is the reader; a UTC stamp would quietly be 5.5 hours wrong. */
  ...
  /* The first line sits 7px under the greeting, the rest 6px under each other —
     the same rhythm the hand-written version had. */
  const bodyRows = copy.lines.map((line, i) =>
    `                <tr><td style="padding:${i === 0 ? '7px' : '6px'} 8px 0;font-family:'Courier New',Courier,monospace;
            ...
            <tr><td style="padding:13px 12px 15px">
              ...
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"
                ...
                <tr><td style="padding:9px 8px 0;font-family:'Courier New',Courier,monospace;
                  ...
                  hey ${escapeHtml(copy.greetingName)} <span style="color:#9c1f52">&#9829;</span>
                </td></tr>
${bodyRows}                <tr><td style="padding:12px 0 0">
                  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"
                  ...
                  <tr><td align="center">
                    <a href="${copy.cta.href}"
                       style="display:block;padding:16px 6px;font-family:'Courier New',Courier,monospace;
                              ...
                              letter-spacing:.08em">${copy.cta.label}</a>
                  </td></tr>
                </table>
              </td>
...
/* ----------------------------------------------------------------------------
   SENDING — one door, so deliverability is decided in one place.
   ...
   Returns { sent } plus, when it failed, a short reason the CALLER decides what
   to do with. It never throws: a mail failure must not fail the write that came
   before it, because the row is the part that matters.
   -------------------------------------------------------------------------- */
export async function sendHandsetEmail(
  to: string,
  ...
  const replyTo = Deno.env.get('MAIL_REPLY_TO') ?? 'hello@secretslumberparty.com';
  try {
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        /* The wordmark travels WITH the message rather than being fetched from
           the site. content_id is what binds it to src="cid:slumberlogo". */
        attachments: [{
          filename: 'logo.png',
          content: LOGO_B64,
          content_id: LOGO_CID,
    ...
    /* Resend's message names the sending domain and the account state, so it is
       for the log and never for the response body. */
    const why = (await res.text()).slice(0, 400);
    console.error('resend rejected', res.status, why);
    return { sent: false, reason: `${res.status} ${why}` };
  } catch (e) {
    console.error('resend threw', e);
    return { sent: false, reason: 'threw: ' + String(e).slice(0, 300) };
  }
}
